...
> 최신 릴리즈: **v0.4.4 — Windows SDR 밝기에 맞춘 좌우 비교**
...
비교 파일 전체는 BT.2020/PQ HDR로 저장됩니다. **Windows 자동**은 변환 시작 시 GUI가 있는 HDR 화면의 Windows SDR 흰색 밝기를 읽어 왼쪽 SDR에 적용합니다. 이전 v0.4.3의 203 nits 고정값 때문에 Windows에서 SDR을 더 밝게 보는 환경에서는 비교 영역이 지나치게 어두웠던 문제를 수정했습니다. 자동을 끄면 **SDR nits**를 80~1000으로 직접 지정할 수 있고, 자동/수동 선택과 수동 값은 재실행 후에도 유지됩니다. HDR 화면의 값을 읽지 못하면 `자동 (203 기본)`을 표시합니다. 작업 재개 시에는 당시 저장한 밝기를 사용합니다. 오른쪽 HDR 효과는 바꾸지 않습니다. 다른 화면이나 SDR로 톤 매핑하는 플레이어에서는 보이는 밝기가 달라질 수 있으므로 실제 재생 환경에 맞춰 조절하세요. 해상도·프레임률과 MKV/MP4별 오디오 처리는 유지되며, 비교용 GPU 처리 비용은 추가됩니다. 비교 체크박스 자체는 재실행 시 꺼집니다.
...
| `--sdr-white-nits N` | 비교 SDR 흰색 밝기(80~1000 nits). 생략하면 Windows 주 화면 기준, 감지 실패 시 203 |
