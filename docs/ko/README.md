...
> 최근 공개 릴리즈: **v0.4.1 — 구간 저장 선택·중단 작업 재개·오류 진단 보강**
> 현재 소스/로컬 빌드: **v0.4.2-dev — 성공한 작업의 중간 영상 자동 정리**
...
5. **결과 저장 완료 · 중간 파일 정리 중** — v0.4.2-dev 빌드에서 큰 임시 파일을 삭제하고 로그를 남깁니다.
...
- 현재 개발 빌드는 오디오 결합·결과 검사·최종 저장이 모두 성공하면 중간 영상과 큰 원시 프레임 파일을 자동 삭제합니다. 로그와 작은 작업 메타데이터는 보존합니다. 처리 중에는 구간 합계와 최종 파일에 해당하는 추가 디스크 공간이 필요합니다.
...
### 정상 완료 후 중간 파일 자동 정리 (v0.4.2-dev)
